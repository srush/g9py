...
def uncmin(params, loss, affects, end_on_line_search=True, tol=1e-8, maxit=1000):
    ...
    f0.backward()
    def construct_grad():
        g0 = np.array([0.0 if p.grad is None or (affects is not None and (n not in affects or (isinstance(affects[n], minitorch.Tensor) and affects[n][i] == 0))) else p.grad[i]
                       for n, p in params
    ...
    for it in range(maxit):
        # if not g0.every(isFinite):
        #     msg = "Gradient has Infinity or NaN"
        #     break
        step = -(H1 @ g0)
        # if not step.every(isFinite):
        ...
        while it < maxit and t * nstep >= tol:
            ...
            for _, p in params:
                ...
                i += len(v)
            f1 = loss()
            if not (f1 - f0)[0] >= 0.1 * t * df0: # or isNaN(f1)):
                break
            t *= 0.5
            it += 1
        ...
        H1 = (H1 + (out(s, s) * (ys + (dot(y, Hy))) / (ys * ys))) - ((out(Hy, s)) + (out(s, Hy))) / ys
        x0 = x1
        f0 = f1
        g0 = g1
        # print("update", time.time()-stime2)
    # print("opt", time.time() - stime)
    # print("msg", it, msg)
    return {"solution": x0,
            "f": f0,
            "gradient": g0,
...
# def makeDraggable(el, startDrag, drag):
#     startex, startey, startx, starty = None
...
#         startx = startx if startx is not None else 0 
#         starty = starty if starty is not None else 0 
...
class G9:
    def __init__(self, module, onChange= lambda: {}):
        ...
        self.parent = None
        self.xAlign = 'center'; self.yAlign = 'center'
        self.width = 0; self.height = 0; self.top = 0; self.left = 0
    ...
    def resize(self, rerender = True):
        ...
        self.node.setAttribute('viewBox', " ".join(map(str, [-self.xOffset, -self.yOffset, r.width, r.height])))

        self.g9Offset = {
            "top": r.top + self.yOffset,
            "left": r.left + self.xOffset
        }
        if rerender:
            self.render()
    ...
    def render(self):
        ...
        for id, renderable in self.renderables.items():
            if id not in self.elements:
                # print(id)
                self.elements[id] = Point(g9=self) if renderable["type"] != "line" else Line(g9=self)
